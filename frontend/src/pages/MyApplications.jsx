          ...
          {apps.length === 0 ? (
            ...
            apps.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className="bg-gray-50 dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-200 p-6 border border-gray-200 dark:border-gray-700"
                >
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex-1 mb-4 lg:mb-0">
                      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{a.job?.title || a.job_title}</h3>
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(a.status)}`}>
                        ...
                        {a.status === "pending" ? "Menunggu" : a.status === "accepted" ? "Diterima" : a.status === "rejected" ? "Ditolak" : a.status}
                      </span>
                    </div>
                  </div>
                </div>
