...
@view_config(route_name='applications', request_method='POST', renderer='json', permission='apply_job')
...
def get_my_applications(request):
    ...
    return [{'id': a.id, 'job_id': a.job_id, 'job_title': a.job.title, 'status': a.status, 'applied_date': str(a.applied_date)} for a in apps if a.job]
...
def update_application_status(request):
    ...
    if not app or app.job.employer_id != int(user_id):
